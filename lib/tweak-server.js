...
router.get('/deploy', function(req, res) {
    ...
    fs.remove(path.resolve("public/deploy"), err => {
        if (err) return console.error(err)
        var zipFolder = require('zip-folder');
        zipFolder(path.resolve("public/"), 'deploy.zip', function(err) {
            if (err) {
                res.status(404).send('Could not generate zip file!');
            } else {
                fs.ensureDir(path.resolve("public/deploy"));
                fs.move(path.resolve("deploy.zip"), path.resolve("public/deploy/deploy.zip"), function(err) {
                    if (err)
                        console.error(err);
                    res.status(200).send('Deploy zip generated!');
                });
            }
        });
    });
